...
- **[[3.Case Sensitivity|Case Sensitivity]]**: Unlike Windows, the Linux filesystem is case-sensitive. This means `File.txt` and `file.txt` are viewed as two entirely different entities.
- **[[4.Directory|Directory]]**: This term has two distinct meanings in Linux:
...
- **[[8.Shell(bash) vs Terminal(cli)|Shell vs. Terminal]]**:
...
- **[[7.Script|Script]]**: These are files containing a series of commands executed by an interpreter (like Bash or Python). Many hacking tools are essentially complex scripts.
...
- **[[6.Root|/root]]**: The home directory for the superuser (not to be confused with `/`).
- **/etc**: Contains the configuration files that control how the system and programs start.
- **[[5.Home|/home]]**: Where standard user files are kept.
